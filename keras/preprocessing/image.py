...
def random_shift(x, wrg, hrg, row_axis=1, col_axis=2, channel_axis=0,
                 fill_mode='nearest', cval=0., seed=None):
        ...
        if augment:
            ...
            for r in range(rounds):
                for i in range(x.shape[0]):
                    ax[i + r * x.shape[0]] = self.random_transform(x[i], 0)
            x = ax
...
class NumpyArrayIterator(Iterator):
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            ...
            x = self.image_data_generator.random_transform(x.astype(K.floatx()), self.seed)
...
class DirectoryIterator(Iterator):
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            ...
            x = self.image_data_generator.random_transform(x, self.seed)
            np.random.seed(self.seed)
            x = self.image_data_generator.standardize(x)
            batch_x[i] = x
